...
func NextRespData(r io.Reader) (respModels.RespData, error) {
...
func NextSimpleInt(r io.Reader) (respModels.RespData, error) {
	...
	return respModels.SimpleInt(parseInt), nil
}

func NextSimpleString(r io.Reader) (respModels.RespData, error) {
	line, err := support.NextLine(r, respTerminator)
	if err != nil {
		return nil, err
	}

	return respModels.SimpleString(line), nil
}
...
func NextBulkString(r io.Reader) (respModels.RespData, error) {
	...
	return respModels.BulkString(data), nil
}
...
func NextArray(r io.Reader) (respModels.RespData, error) {
	length, err := support.NextLineAsInt(r, respTerminator, 10, 64)
	if err != nil {
		return nil, err
	}
	...
	data := make([]respModels.RespData, length)
	for i := int64(0); i < length; i++ {
		data[i], err = NextRespData(r)
	}

	return respModels.Array(data), nil
}
